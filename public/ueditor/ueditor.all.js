...
//                });
         ...
         * @grammar editor.getPlainTxt()  => String
         */
        getPlainTxtEx: function () {
            var reg = new RegExp(domUtils.fillChar, 'g'),
                html = this.body.innerHTML.replace(/<a(\s*?\b[^>=]+=[^>=]+)*?(\s*?\bhref=[^>=]+)(\s*?\b[^>=]+=[^>=]+)*?>[^<]+<\/a>/gi,function(str){var dom = $(str);return "<a href=\""+dom.attr('href')+"\">"+dom.html()+"</a>"})
                     .replace(/<\/a>/gi,'###etopshine###').replace(/[\n\r]/g, '');//ie要先去了\n在处理
            html = html.replace(/<(p|div)[^>]*>(<br\/?>|&nbsp;)<\/\1>/gi, '\n')
                .replace(/<br\/?>/gi, '\n')
                .replace(/<[^>/]+>/g, '')
                .replace(/(\n)?<\/([^>]+)>/g, function (a, b, c) {
            ...
            //取出来的空格会有c2a0会变成乱码，处理这种情况\u00a0
            return html.replace(reg, '').replace(/\u00a0/g, ' ').replace(/&nbsp;/g, ' ').replace(/###etopshine###/g,'</a>');
        },
        /**
         * 获取编辑器中的纯文本内容,没有段落格式
         * @name getContentTxt
