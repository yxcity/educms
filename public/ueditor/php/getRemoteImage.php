<?php
	 ...
	 * 
	 */
	function _setSavePath()
	{
		$host = strchr($_SERVER ['HTTP_HOST'],".");
		$subDomain = str_replace($host,"",$_SERVER ['HTTP_HOST']);
		if(!isset($_COOKIE["_identify"])){
			$url = "login".$host;
			header("Location:$url");
			exit(0);
		}
		$userId = str_replace($subDomain,"",$_COOKIE["_identify"]);
		$dirname = "uploads/".substr(md5($userId),0,10)."/editor/".date("Ymd")."/";	
		return $dirname;
	}
