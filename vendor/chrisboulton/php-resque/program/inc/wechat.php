...
class Wechat
{
	...
	public function getContactsPages($pagesize=10,$pageidx=0,$groupid=0,$nav=false)
        {
            $send_snoopy = new Snoopy; 
            $send_snoopy->rawheaders['Cookie']= $this->cookie;
            $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/indexpage?t=wxm-index&lang=zh_CN&token=".$this->_token;
            $url = "https://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&lang=zh_CN&groupid=${groupid}&pagesize=${pagesize}&pageidx=${pageidx}&token=".$this->_token;
            $send_snoopy->fetch($url);
            $result = null;
            if (preg_match('/<script[^>]*?id="json-friendList"[^>]*?>([\s\S]*?)<\/script>/i', $send_snoopy->results, $match)) {
                $result =array('data'=>json_decode($match[1], true));
                $result['nav'] = null;
                if ($nav && preg_match('/DATA\.List\.pageNav\s*=\s*({[^}]+})/i', $send_snoopy->results, $match)) {
            ...
            logger(json_encode($result),'webchat',LOG_DEBUG);
            if(!$result){
                return false;
            }
            return $result;
    ...
    */
    function getUnsyncMembs($is_full_scan,$callback)
    {
        ...
        if(!($ret && isset($ret['nav']['PageCount']))){
            logger("getContactsPages error.","webchat",LOG_DEBUG); 
            return false;
        }
        
        $pages = (int)$ret['nav']['PageCount'];
        if($pages <=0){
            logger("No users to sync.","webchat",LOG_INFO); 
            return true;
        }
        logger("Find almost" .($pages * $size). " users.\n","webchat",LOG_INFO); 
        for($i=0;$i<$pages;$i++){
            ...
            foreach($data['data'] as &$item){
                array_push($ids,$item['fakeId']);
                $item['detail'] =  $this->getInfo($item['fakeId']);
            }
            if(count($ids)){
    ...
    */
    function syncGroupMemb($groud_id,$callback,$archive_group=-1,$size=200)
    {
        ...
        $pages = (int)$ret['nav']['PageCount'];
        if($pages <=0){
            logger("there is no users to sync from in group:${groud_id}.","wechat",LOG_INFO); 
            return true;
        }else{
            logger("find " .($pages). " users in group:${groud_id}.","webchat",LOG_INFO); 
        }
        $pages = (int)($pages/$size) + ($pages%$size?1:0);
        for($i=0;$i<$pages;$i++){
            $data = $this->getContactsPages($size,$i,$groud_id,false);
            logger("reading group:$groud_id,page:$i,pagesize:$size,read count:".count($data['data']),"webchat",LOG_INFO); 
            $ids = array();
            $detail_info = array();
            foreach($data['data'] as &$item){
                array_push($ids,$item['fakeId']);
                //$item['detail'] =  $this->getInfo($item['fakeId'],$groud_id);
            }
            if(count($ids)){
    ...
    */
    function getUnsyncMsgs($connection,$callback)
    {
        ...
        if($total <=0){
            logger("No message to sync.","webchat",LOG_INFO); 
            return true;
        }
        logger("Find $total Messages.","webchat",LOG_INFO); 
        for($i=0;$i<$pages;$i++){
            /*moveGroup会更新分页，因此每次更新第一页*/
    ...
    function getGroups()
    {
        ...
        $send_snoopy->rawheaders['Cookie']= $this->cookie;
        $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/indexpage?t=wxm-index&lang=zh_CN&token=".$this->_token;
        $url = "https://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&lang=zh_CN&groupid=0&pagesize=&pageidx=&token=".$this->_token;
        $send_snoopy->fetch($url);
        $result = null;
        //$this->logger($send_snoopy->results);
        if (preg_match('/DATA\.groupList\s*=\s*([^;]+);/i', $send_snoopy->results, $match)) {
            $groups = array();
	...
	private function logger($logger){
		if ($this->debug && function_exists($this->_logcallback)) {
			if (is_array($logger)) $logger = print_r($logger,true);
			return call_user_func($this->_logcallback,$logger);
		}
	}
